# Juscribe Workflow SOP — OpenAI Codex (CLI / IDE / app)

This bundle ships two on-demand Agent Skills that codify the Juscribe ticket-management workflow. Skills auto-invoke when their `description` field matches the user's intent; you can also call them directly. Works in Codex CLI, the Codex IDE plugins (VS Code / JetBrains), and the Codex app — all three surfaces read the same `SKILL.md` files via the [Agent Skills standard](https://agentskills.io).
...
- **Hooks** (`jus/hooks/`) are Claude Code-specific and have no equivalent in Codex. The skill prompts in `hard-rules` cover the same intent on a best-effort basis.
- **`allowed-tools`** in skill frontmatter is a Claude Code allowlist hint — Codex ignores it.
- **Custom prompts** (Codex's pre-skills mechanism) are deprecated upstream in favor of skills; this bundle does not ship any.
