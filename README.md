...
| `jus-dirty-tree-nudge.sh`                         | `PostToolUse Edit/Write`        | After 5 uncommitted edits, emits a system-message reminder to commit (non-blocking). Counts the worktree this session locked when there is one — see _Which tree the commit guards read_                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `jus-stop-uncommitted.sh`                         | `Stop`                          | Prevents the session from ending while the working tree is dirty. Checks the worktree this session locked when there is one — see _Which tree the commit guards read_                                                                                                                                                                                                                                                                                                                                                                                                                                             |

### Which tree the commit guards read

`jus-stop-uncommitted.sh` and `jus-dirty-tree-nudge.sh` both need to know which checkout is _yours_. They take it from the `cwd` the harness hands them — **unless** a `git worktree` is locked with a reason carrying the session id, in which case that worktree is the tree they read.

This matters for one workflow in particular: an agent that works in a worktree but keeps its `cwd` at the main checkout, `cd`ing in per command. Its `cwd` then names a tree it never edits, so without the lock the stop hook blocks on somebody else's uncommitted files and the nudge counts them.
