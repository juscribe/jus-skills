...
| ---------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------- |
| Claude Code                                          | ✅ via `.claude-plugin/plugin.json`                                                                                                                | ✅                                                                                                                                              | `claude --plugin-dir ./jus` or `/plugin marketplace add juscribe/jus-skills` → `/plugin install jus@jus-skills`                            | `checked:claude 2026-09-15`      |
| Google Antigravity CLI (`agy`)                       | ✅ via `.agents/skills/` (Gemini CLI sunset 2026-06-18)                                                                                            | ✅ via `hooks/antigravity/` adapter — **live-verified** with a control arm; install at `~/.gemini/config/hooks.json`, NOT project scope (#4262) | canonical `.agents/skills/` install; `agy plugin import gemini` for the legacy ext                                                         | `checked:antigravity 2026-09-17` |
| Antigravity IDE (desktop)                            | ✅ via `.agents/skills/` (Gemini Code Assist IDE sunset 2026-06-18)                                                                                | ⚠️ same `hooks/antigravity/` adapter; verified on the CLI only (#4262)                                                                          | canonical `.agents/skills/` install (same project path as the CLI)                                                                         | `checked:antigravity 2026-09-17` |
| OpenAI Codex (CLI / IDE / app)                       | ✅ via `.agents/skills/` (documented roots; same `SKILL.md`)                                                                                       | ✅ via `hooks/codex/` adapter (same scripts, Codex-native manifest + trust flow)                                                                | canonical `.agents/skills/` install + `hooks/codex/README.md`                                                                              | `checked:codex 2026-09-15`       |
| Cursor                                               | ✅ via Skills surface (uses same `SKILL.md`)                                                                                                       | ⚠️ via `hooks/cursor/` adapter — **written, not live-verified**; `stop` cannot block, so the dirty-tree gate is advisory (#4261)                | canonical `.agents/skills/` install (`.cursor/skills/` also read); Marketplace listing via `.cursor-plugin/plugin.json` pending submission | `checked:cursor 2026-09-16`      |
| Kimi Code (CLI / VS Code / ACP)                      | ✅ via `.agents/skills/` (also `.kimi-code/skills/`; does **not** read `.claude/skills/`)                                                          | ✅ via `hooks/kimi-code/` adapter or the `kimi.plugin.json` plugin (PostToolUse nudges degrade — see its README)                                | Kimi plugin (`kimi.plugin.json` — skills + hooks + session-start hard-rules) or canonical `.agents/skills/` install                        | `checked:kimi 2026-09-15`        |
| GitHub Copilot (coding agent / CLI / IDE agent mode) | ✅ via `.agents/skills/` (also `.github/skills/`, `.claude/skills/`; user scope `~/.copilot/skills/` + `~/.agents/skills/`)                        | ⚠️ via `hooks/copilot/` adapter — **written, not live-verified** (#4260)                                                                        | `gh skill install juscribe/jus-skills` (gh ≥ 2.90, preview) or canonical `.agents/skills/` install                                         | `checked:copilot 2026-09-16`     |
| Qwen Code (`qwen` CLI)                               | ✅ via `.qwen/skills/` + `~/.qwen/skills/` — ⚠️ **NOT `.agents/skills/`** ([#2042](https://github.com/QwenLM/qwen-code/issues/2042) closed, no PR) | ✅ via `hooks/qwen/` adapter — exit 2 blocks and `Stop` blocks, so no degradation; **not live-verified** (#4263)                                | `.qwen/skills/` symlinks (the canonical `.agents/` recipe alone loads NOTHING here) + `hooks/qwen/README.md`                               | `checked:qwen 2026-09-16`        |
| Windsurf (now Devin Desktop)                         | ✅ via `.agents/skills/` (native skills)                                                                                                           | ⚠️ via `hooks/windsurf/` adapter — **unverified, and NOT verifiable headlessly**; Cascade has no `Stop` event at all (#4264)                    | canonical `.agents/skills/` install                                                                                                        | `checked:windsurf 2026-09-16`    |
| Zed                                                  | ✅ via `.agents/skills/` (native skills)                                                                                                           | ❌                                                                                                                                              | canonical `.agents/skills/` install                                                                                                        | `checked:zed 2026-09-15`         |
